...
class pyscf_object(object):
    ...
    def get_hao(self):
        ...
        _R = np.dot(vh.T, u.T) # transformation matrix, { \psi } = { \phi }*A
...
def get_v12_sp3(cs): # sp3 O
    ...
    theta = np.arccos(c12) # np.pi*93./180. #
...
class config_adapted_hao(RawMol):
    ...
    def __init__(self, zs, coords, basis='cc-pvdz', ihao=T): # fn='test'):
...
class pairwise_rotate(RawMol):
    ...
    def get_max_ovlp_bst(self, iac):
       ...
       ibs, ies = t[:,0], t[:,1]
       idx = [ list(range(ibs[i],ies[i])) for i in range(na) ]
       ...
       idx2 = np.concatenate([ list(range(ibs[i],ies[i])) for i in ias[g[iac]>0] ])
       ...
       for it in range(2048):
          ...
          for i in range(nbf):
             for j in range(i):
                ...
                _idx2 = np.concatenate([ list(range(ibs[ia],ies[ia])) for ia in ias2 ])
...
if __name__ == '__main__':
  ...
  for fn in fns:
    ...
    v = [1.,1.,1.]
    if a != 0.0:
        m.rotate(a,v) # a*np.pi/180.) # rotate `a degree around vector `v
    #"""
