...
def FindAllPathsOfLengthMToN_Gobbi(mol, minlength, maxlength, rootedAtAtom=-1, uniquepaths=True):
  ...
  for atom in mol.GetAtoms():
    if rootedAtAtom == -1 or atom.GetIdx() == rootedAtAtom:
      ...
      visited = set()
      _FindAllPathsOfLengthMToN_Gobbi(mol, atom, path, minlength, maxlength, visited, paths)
...
def _FindAllPathsOfLengthMToN_Gobbi(m, atom, path, minlength, maxlength, visited, paths):
  for bond in atom.GetBonds():
    if bond.GetIdx() not in path:
      ...
      nodes = set([])
      for ib in path:
        bi = m.GetBondWithIdx(ib)
        ab, ae = bi.GetBeginAtom(), bi.GetEndAtom()
        nodes.update( [ab.GetIdx(), ae.GetIdx()] )
      #print('nodes = ', nodes, 'path = ', list(path))
      spath = set(path)
      if len(path) >= minlength and len(path) <= maxlength:
        if spath in paths: continue
      ...
      if len(path) < maxlength:
        ...
        if nextatomidx not in visited:
          ...
          _FindAllPathsOfLengthMToN_Gobbi(m, nextatom, path, minlength, maxlength, visited, paths)
          #visited.remove(nextatomidx)
# previously one line below doen't exist; by adding it we get exactly the same results as
# `rdkit FindAllPathsOfLengthMToN function
          #_FindAllPathsOfLengthMToN_Gobbi(m, atom, path, minlength, maxlength, visited, paths)
          visited.remove(nextatomidx)
      path.pop()
...
def find_all_paths_of_M_to_N_vertexes(mol, namin, namax, \
                  rootedAtAtom=-1, uniquepaths=True):
  tpaths = []
  ...
  for atom in mol.GetAtoms():
    if rootedAtAtom == -1 or atom.GetIdx() == rootedAtAtom:
      ...
      visited = set([atom.GetIdx()])
      #     visited = set()
      _find_all_paths_of_M_to_N_vertexes(atom, path, nodes, namin, namax, visited, paths)
      tpaths += paths
  ...
  for atom in mol.GetAtoms():
    if rootedAtAtom == -1 or atom.GetIdx() == rootedAtAtom:
      ...
      visited = set([atom.GetIdx()])
      #     visited = set()
      _find_all_paths_of_M_to_N_vertexes_extra(atom, path, nodes, namin, namax, visited, paths)
      tpaths += paths
  ...
  if uniquepaths:
    ...
    for spath in paths:
      if spath not in seen:
        uniquepathlist.append( list(spath) )
        seen.append(spath)
    uniquepathlist.sort()
    return uniquepathlist
  else:
...
def _find_all_paths_of_M_to_N_vertexes(atom, path, nodes, namin, namax, visited, paths):
  for bond in atom.GetBonds():
    if bond.GetIdx() not in path:
      bidx = bond.GetIdx()
      a1 = bond.GetBeginAtom()
      a2 = bond.GetEndAtom()
      nodes.update( [a1.GetIdx(), a2.GetIdx()] )
      path.append(bidx)
      if len(nodes) >= namin and len(nodes) <= namax:
        spath = set(path)
        #if spath in paths: continue
        paths.append(spath)
      if len(nodes) <= namax:
        ...
        if nextatomidx not in visited:
          visited.add(nextatomidx)
          _find_all_paths_of_M_to_N_vertexes(nextatom, path, nodes, namin, namax, visited, paths)
# previously one line below doen't exist; by adding it we get exactly the same results as
# `rdkit FindAllPathsOfLengthMToN function
          #_find_all_paths_of_M_to_N_vertexes(atom, path, nodes, namin, namax, visited, paths)
          visited.remove(nextatomidx)
...
def _find_all_paths_of_M_to_N_vertexes_extra(atom, path, nodes, namin, namax, visited, paths):
  for bond in atom.GetBonds():
    if bond.GetIdx() not in path:
      bidx = bond.GetIdx()
      a1 = bond.GetBeginAtom()
