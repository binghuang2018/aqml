...
import cml.representation.core as rc
import pickle as pkl

#try:
import aqml.cheminfo.rdkit.core as coc
import aqml.cheminfo.rdkit.rdkit as coo
#except:
...
class ExtM(object): #coc.newmol):
    ...
    def generate_coulomb_matrix(self,inorm=False,wz=False,rpower=1.0):
        ...
        self.cm = rc.py_generate_cm(self.zs, self.coords, inorm=inorm,
                        wz=wz, rpower=rpower)
...
class ParentMol(coo.StringM):
    ...
    def __init__(self, string, stereo=F, isotope=F, iat=None, i3d=T,\
                 ...
                 iextl=F, ivdw=F, inmr=F, debug=F, iwarn=T, verbose=1, \
        ...
        if os.path.exists(string) and self.na > 100: # save `ds and `pls
            ...
            if os.path.exists(ftmp):
                if self.verbose: print(' ++ read `ds and `pls from file: ', ftmp)
                dt = np.load(ftmp)
                self._ds = dt['ds']
                self._pls = dt['pls']
            else:
                ...
                if self.verbose: print(' ++ save `ds and `pls to file ', ftmp)
    ...
    def i_ct0ex2vb_persist(self, iasq, newm):
        ...
        if self.ats_ict0ex2vb:
            if self.verbose: print('    Found in query conj ats w dang=0 & 2 end atoms rij < sum of rvdw')
            _atsq = []
            for _atsi in newm.ats_ict0ex2vb:
                _atsq.append( iasq[_atsi] )
            if np.any([ set(_atsj) in self.ats_ict0ex2vb for _atsj in _atsq ]):
                if self.verbose: print('    And this amon contains such local env')
    ...
    def generate_amons(self,submol=None):
        ...
        for seed in generate_subgraphs(self.b2a, self.a2b, k=self.k, submol=submol):
            ...
            # initial SMILES
            #atsi = [ int(iia) for iia in lasi ]
            #print('atsi=', atsi, 'mol=',self.mol)
            #can0 = Chem.MolFragmentToSmiles(self.mol, atomsToUse=atsi) #
                                            #atomsToUse=atsi, \
                                            #bondsToUse=bs)
            #print('can0=', can0)
...
# By doing This, we can save a lot of work for BO perception later!
            ...
            for _bom in boms:
                ...
                try:
                    ...
                    if self.verbose:print(si)
                    raise Exception('#ERROR: mol to can failed')

                if '.' in can:
                    continue

                if self.verbose:print(' ## can = ', can)
                ...
                if ifd_extl: # and (can not in cans_u):
                    if self.verbose:print(' ##### found larger essential amons with N_I=%d: %s'%(nheav, can))
                    #ishown = F
                    ...
                    if np.all( np.logical_and(newm.iconjsv>0, newm.iconjsv<=2) ):
                        if len(ats_cr5_a) > 0:
                            ...
                            if newm.gv[ats_cr5_a][:,ats_nr_a].sum()==1: #(newm.iconjs[ats_nr_a]!=1).sum() >= 1:
                                if self.verbose:print('    # N_I>7 & 5-ring, found only one -R group, skip!')
                                continue

                        if (newm.iconjsv==2).sum()>2:
                            if self.verbose:print('   # N_I>7, found >2 atoms with iconj==2, skip!')
                            continue

                        #if len(newm.ats_ict0ex2vb)>=3:
                        if len(newm.ats_ic4ex2c)>=3:
                            if self.verbose:print('   # N_I>7, found tors that are `ic4ex2c >=3 times, skip!')
                            continue
                    else:
                        if self.verbose:print('   # N_I>7, but some atom has iconj=0 or >2, skip!')
                        continue
                ...
                if len(ats_cr5_a)>0:
                    if self.verbose:print('    # found 5-membered aromatic ring')
                    istop = F
                    for ia5 in ats_cr5_a:
                        ...
                        if len( np.intersect1d(_lasi[nbrs_i], ats_meso_q) ) >0: # ar: aromatic ring
                            if self.verbose:print('    found >=1 atoms (in 6-ar of Q) attached to 5-ar, skip!')
                            istop = T
                            break
                    if istop:
                        continue
                ...
                if nheav>7 and newm.iconjr:
                    ...
                    if newm.aromatic.sum() != iarsq.sum():
                        if self.verbose:print(' #### %s aromatic states of some atoms have changed, skip!'%can)
                        continue
                    na_meso = imeso_a.sum()

                    if np.all(imesoq) and (na_meso==0):
                        if can not in ['C=c1ccccc1=C']: ###### need for a final call to determine if this is necessary
                            if self.verbose:print('    # found C=c1ccccc1=C or structure alike, skip!' )
                            continue
                        #print('    imeso_a = ', imeso_a, 'imesoq=', imesoq)

                    if na_meso > 0:
                        if na_meso != imesoq.sum():
                            if self.verbose:print(' #### %s : mesomeric state of some atoms changed, skip!'%can)
                            continue
                        else:
                                ...
                                if ifd:
                                    ...
                                    if np.all(imeso[idxs[0][-2:]]):
                                        ...
                                        if set(newm.zs[newm.gmeso.ats_meso]) != set([6]):
                                            if self.verbose:print('    found hetero-nuc meso ring (e.g., c1cccnc1), geom turns flat after optg, skip!')
                                            continue
                                        if self.verbose:print('    where b) some tor has an angle > 30 degree, keep it!')
                                    elif np.all([ set(i4s) not in sets for i4s in idxs ]):
                                        # E.g., Ph-CH=NH, where H points outwards of the plane formed by ats_v
                                        if self.verbose:print('    where no `ats_ic4ex2c exists, keep it!')
                                    else:
                                        if self.verbose:print('    neither a) or b) is satisfied, skip!')
                                        continue
                        ...
                        if not np.all(self.iconjs[atsi]):
                            if nshr > ndic_nonconj[nar] and nshr < nar:
                                ...
                                if np.any(self.istrains[share] != newm.istrains[iaas]):
                                    ...
                                    break
                if not iok:
                    if self.verbose:print('## strain status changed: ', can)
                    continue
                ...
                if self.icc4Rsp3out:
                    ...
                    if newm.ats_cc4bent and newm.i_ats3_conn_cc4bent:
                        if self.verbose:print('   found \\__// and its derivatives, skip!')
                        continue
                ...
                if self.i3d and self.nocrowd:
                    if newm.icrowd_cov:
                        ...
                        if self.verbose:print('    Crowded due to too close cov bond, geom to ./crowded/%s'%fot)
                        continue
                    ...
                    if self.nogc: # no geometry clash due to H's attached to ats_ict0ex2
                        ...
                        if ats_a:
                            if ats_q:
                                ...
                                iclash = T
                        if iclash:
                            if self.verbose:print('   geometry clash found in amons, but not in query. Skip!')
                            continue
                ...
                if self.irddtout:
                    if newm.irddt:
                        if self.verbose:print('    mol is too flexible')
                        if not self.i3d:
                            continue
                        else:
                            if newm.irigid:
...
############################################################
                        ...
                        if self.i3d:
                            ...
                            #    continue
                            #else:
                            if self.verbose:print('    mol is rigid and contains no vdw bond. keep!')

                cans_u.append( can )

                if self.verbose: print('    \To be accepted/')
...
class ParentMols(object):
    ...
    def __init__(self, strings, reduce_namons=F, fixGeom=F, iat=None, wg=T, i3d=T, \
                 ...
                 irad=T, ichg=T, prefix='', iwarn=T, debug=F, log=T, verbose=1):
        """
        ...
        for ir in range(nmt):
            ...
            obj = ParentMol(string, iat=iat, i3d=i3d, k=k, k2=k2, stereo=stereo, \
                            ...
                            keepHalogen=keepHalogen, debug=debug, iwarn=iwarn, verbose=verbose, \
            ...
            if i3d and ivdw:
                if substring != None:
                    ...
                    if not iqg:
                        if not use_f90:
                            ...
                            for p in icsa:
                                ...
                                ipts.append( [nprocs, self.k2, obj, mi, mj, verbose] )
                        ...
                        for im in range(nmi):
                            ...
                            if nj > 0:
                                ...
                                if nmaxcomb >= 3:
                                    ...
                                    if verbose: print( '  now search for vdw amon made up of 3 cov amons')
                                    t0 = time.time()
                                    nasv3 = []
                                    for (p,q) in itl.combinations(jms,2):
                                        ...
                                        if _nav <= k2 and gc[p,q] == 0:
                                            ...
                                            if comb not in combs:
                                                ...
                                                combs3 += [comb]
                                                nasv3 += [_nav]
                                    if verbose: print('  done within time: ', time.time() - t0, ' seconds')
                    ...
                    if verbose: print('atom indices of all amons done')

                    if iqf and saveg and (not iqg):
                        with open(fqg, 'wb') as fid:
                            pkl.dump({'gv': gv, 'gc': gc}, fid)
                        if verbose: print(' ++ `gv and `gc saved to file ', fqg)
                    ...
                    for comb in combs:
                        ...
                        if (not fixGeom) and np.any(newmi2.hybsv != obj.hybs[mi2.iasvq]):
                            if verbose: print('    post-screening: found vdw amons %s with changed hyb, skip!'%mi2.can0)
                            continue
        ...
        if not i3d:
            ...
            maps = None
            if imap:
                if verbose: print(' -- now sort amon SMILES by N_I')
                seta._sort2()
                cans = seta.cans
                maps= seta.maps2
                if label is not None:
                    h5f = label + '.pkl' # '.pkl'
                    with open(h5f, 'wb') as fo2:
                        pkl.dump( {'ids': np.array(ids,dtype=int), 'maps': seta.maps2}, fo2 )
            ...
            if not iclean: # debug
                ...
                if wg and nmt>1:
                    with open(h5f, 'wb') as fo1:
                        pkl.dump( {'ids': np.array(ids,dtype=int), 'maps': seta.maps2}, fo1 )
                    #dd.io.save(h5f, {'ids': np.array(ids,dtype=int), 'maps': seta.maps2} )
                ...
                self.nm = sum(ncs)
                if wg:
                    if verbose: print(' amons are to be written to folder %s/'%fdn)
                    ...
                    for i in range(ncan):
                        ...
                        if ivao and verbose:
                            print('%9d   %06d %9d %9d %60s'%(nsheav[i], i+1, nci, sum(ncs[:i+1]), cans[i]))
                        for j in range(nci):
                ...
                self.ncan = ncan
                self.ms0 = ms0
                if verbose: print(' ## summary: found %d molecular graphs, %d configurations'%(ncan, self.nm) )
    ...
    def get_matched_subm(self, ias, itype='f', otype='mol'):
        ...
        if i in self.iass:
            ...
            if verbose: print(' ** no match found, return closest match instead')
            na_share = [ len(set(i).intersection(set(j))) for j in self.iass ]
            seq = np.argsort(na_share)
            idx = seq[-1]
            if verbose: print('        ias = ', self.iass[idx] )
        if otype in ['mol']:
            ot = self.ms[idx]
        elif otype in ['sdf']:
...
def get_conn(ipt):
    ...
    """
    nprocs, k2, obj, mi, mj, verbose = ipt
    pls0 = obj.pls
    g0 = obj.g
                ...
                if np.any( [ set(b) <= set(mi.iasq+mj.iasq) \
                    ...
                    if nbv0 > 0 and nbv0==nbv:
                        if nbv==1: # and nbv0>=1:
                            ...
                            if np.any(ioks):
                                if np.sum(ioks) >= 2:
                                    if verbose: print("   found more than 2 HB's in a conj env", mc.can0, 'keep it!')
                                    vc = 1
                            else:
                                if nbo1 <= 2*nbv-1:
                                    if verbose: print("    found multiple vdw bonds in", mc.can0,  'bsv=',bsv)
                                    vc = 1
    return cc,vc
