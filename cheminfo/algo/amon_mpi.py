...
class ParentMol(coo.StringM):
    ...
    def __init__(self, string, stereo=F, isotope=F, iat=None, i3d=T,\
        ...
        coo.StringM.__init__(self, string, stereo=stereo, isotope=isotope, \
                             ds=ds, pls=pls, scale_vdw=scale_vdw, nprocs=1)
...
class params(object):
    ...
    def __init__(self, string, reduce_namons=F, fixGeom=F, iat=None, wg=T, i3d=T, \
...
def genamons(p):
    ...
    obj = ParentMol(p.string, iat=p.iat, i3d=p.i3d, k=p.k, k2=p.k2, stereo=p.stereo, \
                    ...
                    forcefield=p.forcefield, nprocs=p.nprocs)
    lms = [] # list of mols
    if not obj.istat:
      ...
      try:
          for mi in obj.generate_amons(submol=None):
              #if i3d:
              lms.append(mi)
      except:
...
class ParentMols(object):
    ...
    def __init__(self, strings, reduce_namons=F, fixGeom=F, iat=None, wg=T, i3d=T, \
        ...
        objs = pl.map(genamons, ps)
        for ir in range(nmt):
            print(' im/nm = %d/%d'%(ir+1,nmt))
            lms = objs[ir]
            if len(lms) == 0:
                print('  lms=',None)
                continue
            ...
            #print('  ++ found %d cov amons for mol #%d'%(nmi,ir+1))
