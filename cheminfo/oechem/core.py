...
class newmol(object):
    ...
    """ construct a new oechem mol """

    def __init__(self, zs, chgs, bom, coords=None, ds=None, pls=None, \
                 dsref=None, nprocs=1, scale_cov=1.0, scale_vdw=1.2, debug=F):
        """
        build a OEChem mol object from (zs, chgs, bom, coords)
        ...
        :param dsref:  (as reference) maximally allowed interatomic distance
                        ...
                        determining if the mol is too crowded.
        :type dsref: dict
        """
        ...
        ats = []
        for i in range(na):
            newa = newm.NewAtom( int(zs[i]) )
            newa.SetFormalCharge( int(chgs[i]) )
            #if hybs is not None:
            #    newa.SetHyb( int(hybs[i]) )
            if coords is not None:
                newm.SetCoords(newa, coords[i])
            ats.append(newa)
        for i in range(na):
    ...
    def can(self):
        if not hasattr(self, '_can'):
            self._can = self.get_smiles()
        return self._can

    def get_smiles(self):
        flavor = oechem.OESMILESFlag_Canonical
        return oechem.OECreateSmiString(self.mol, flavor)
    ...
    def write_ctab(self, f, dir='./'):
        ...
        #oechem.OEWriteMolecule(ofs, self.mol)
        crc.write_ctab(self.zs, self.chgs, self.bom, coords=self.coords, sdf=opf)
    ...
    @property
    def iasr345(self):
        if not hasattr(self, '_iasr345'):
            self._iasr345 = tools.reduce(np.union1d, (self.iasr3, self.iasr4, self.iasr5))
        return self._iasr345

    @property
    def iasr3456(self):
        if not hasattr(self, '_iasr3456'):
            self._iasr3456 = tools.reduce(np.union1d, (self.iasr345, self.iasr6))
        return self._iasr3456

    @property
    def iasr56(self):
        if not hasattr(self, '_iasr345'):
            self._iasr345 = tools.reduce(np.union1d, (self.iasr5, self.iasr6))
        return self._iasr345

    @property
    def iasr3(self):
        if not hasattr(self, '_iasr3'):
            iasr3 = np.array([ oechem.OEAtomIsInRingSize(ai,3) for ai in self.mol.GetAtoms() ])
            self._iasr3 = self.ias[ iasr3 ]
        return self._iasr3

    @property
    def iasr4(self):
        if not hasattr(self, '_iasr4'):
            iasr4 = np.array([ oechem.OEAtomIsInRingSize(ai,4) for ai in self.mol.GetAtoms() ])
            self._iasr4 = self.ias[ iasr4 ]
        return self._iasr4

    @property
    def iasr5(self):
        if not hasattr(self, '_iasr5'):
            iasr5 = np.array([ oechem.OEAtomIsInRingSize(ai,5) for ai in self.mol.GetAtoms() ])
    ...
    def iasr6(self):
        if not hasattr(self, '_iasr6'):
            iasr6 = np.array([ oechem.OEAtomIsInRingSize(ai,6) for ai in self.mol.GetAtoms() ])
    ...
    def get_is_conj_amon(self):
        ...
        imth, _idx, _ = is_subg(self.mol, patt, iop=1)
    ...
    def get_strained(self):
        ...
        for ai in self.mol.GetAtoms():
            istrain = False
            if oechem.OEAtomIsInRingSize(ai, 3) or oechem.OEAtomIsInRingSize(ai, 4):
                istrain = True
    ...
    def aromatic(self):
        ...
        if not hasattr(self, '_aromatic'):
            #for ai in self.mol.GetAtoms():
            _aromatic = np.array([ ai.IsAromatic() for ai in self.mol.GetAtoms() ], np.bool)
    ...
    def rdmol(self):
        ...
        if not hasattr(self, '_rdmol'):
            ctab = crc.write_ctab(self.zs, self.chgs, self.bom, self.coords)
    ...
    def icisclose(self):
        ...
        """
        tf = F
        #ts = is_subg(self.mol, '[H][#6,#7;]~?~?[H]', iop=1)[1] # including >N-C=R (R=C<,N-,O)
        return tf
    ...
    def ats_ic4ex2(self):
        ...
        if not hasattr(self, '_ats_ic4ex2'):
            patt = '[^2;!X1]~[^2]~[^2]~[^2;!X1]'
            ifd, idxs, _ = is_subg(self.mol, patt, iop=1)
            ats = []
            for idx in idxs:
                a,b,c,d = idx
    ...
    def get_iasP5orS6(self):
        iasPS = []
        ts = is_subg(self.mol, '[#15,#16;X4]', iop=1)[1]
        for tsi in ts:
            ia = tsi[0]
            if self.hybs[ia] == 3:
                iasPS.append( tsi[0] )
    ...
    def get_iasN3(self):
        ...
        """
        iasN3 = []
        ts = is_subg(self.mol, '[#7;X3;A]', iop=1)[1] # including >N-C=R (R=C<,N-,O)
        for tsi in ts:
            ia = tsi[0]
    ...
    def get_rings(self, namin=3, namax=7, remove_redudant=T):
        ...
        for i in range(namin, namax+1):
            ...
            #else:
            #    pat_i = '*:1' + ':*'*(i-2) + ':*1'
            ifd, idxs, _ = is_subg(self.mol, pat_i, iop=1)
            for idx in idxs:
                if idx:
                    #print('idx=',idx)
                    _set = set(idx)
    ...
    def get_shortest_path(self, i, j):
        ...
        a1 = self.mol.GetAtom( oechem.OEHasAtomIdx(i) )
        a2 = self.mol.GetAtom( oechem.OEHasAtomIdx(j) )
        path = []
        for ai in oechem.OEShortestPath(a1,a2):
            ia = ai.GetIdx()
            path.append(ia)
        return path
    ...
    def get_pls2(self):
        """
        get shortest path length between heavy atoms first, then hydrogen atoms
        May be more efficient compared to `get_pls() for large mol?
        """
        m = self.clone()
        oechem.OESuppressHydrogens(m, F, F, F)
        nha = m.NumAtoms()
        ...
        for ai in m.GetAtoms():
            ...
            for aj in m.GetAtoms():
                ja = aj.GetIdx()
                if ja > ia:
                    pls[ia,ja] = pls[ja,ia] = oechem.OEGetPathLength(ai,aj)
    ...
    def copy(self):
        mc = oechem.OEGraphMol()
        oechem.OECopyMol(mc, self.mol)
        return mc
    ...
    def iasdha(self):
        ...
        if hasattr(self,'_iasdha'):
            return self._iasdha
        ifdhbd, _iashbd, _ = is_subg(self.mol, sma_hbd, iop=1)
        ifdhba, _iashba, _ = is_subg(self.mol, sma_hba, iop=1)
    ...
    def get_ocs(self):
        patt = '[#6;X3]=O'
        _, idx, _ = is_subg(self.mol, patt, iop=1)
        return idx
...
def is_subg(t, q, qidxs=[], woH=False, iop=0, rule='none', kekulize=False):
    ...
    ss = oechem.OESubSearch(q)
    iok = oechem.OEPrepareSearch(m, ss)
