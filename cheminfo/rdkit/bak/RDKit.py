...
#        assert np.all(zs[list(zs).index(1)+1:] > 1)
    ...
    def get_angles_csp2(self):
        ...
        matches = self.m.GetSubstructMatches(q)
        iass3 = []
    ...
    def get_dihedral_angles(self, wH=F, new_ias=None):
        ...
        for match in matches:
            ...
            for b1 in aj.GetBonds():
                ...
                for b2 in ak.GetBonds():
                    ...
                    if (l == i):
                        continue
            # now sort angle types
            zi,zj,zk,zl = [ zs[_] for _ in [i,j,k,l] ]
            ias4 = [i,j,k,l]
            ...
            dang = rdMolTransforms.GetDihedralRad(c1, ias4[0],ias4[1],ias4[2],ias4[3])
            if typez in dic.keys():
                dic[typez] += [dang]
            else:
                dic[typez] = [dang]
        return dic
    ...
    def get_angles(self, wH=False, new_ias=None):
        m2 = self.m
        if self.ih and (not wH):
            self.get_backbone()
            m2 = self.mbb
