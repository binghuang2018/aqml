...
__all__ = ['smi2patt', 'get_bom', 'dsHX', ]

global cnsDic, dic_bonds, h2kc, c2j, dsHX, dic_hyb
...
#        assert np.all(zs[list(zs).index(1)+1:] > 1)
    ...
    def get_angles_csp2(self):
        ...
        This is necessary when optimizing geometry: it may happen that 
        the initial geometry of C=CC=C is totally insane but a local minima
        (determined by forcefield), i.e., angle(H-C-C) = 90.
        By setting a reasonable range of these angles can circumvent this problem.
        """
    ...
    def get_descent_geometry(self, label=None):
        ...
        for ai in self.atoms:
            symbol = chemical_symbols[ ai.number ]
            xyz = ai.position
            s += ' {0:2} {1} 1 {2} 1 {3} 1\n'.format(symbol, *xyz)
        if label is None:
    ...
    def get_dihedral_angles(self, wH=F, key='z'):
        ...
        """
        mol = self.m
        if self.ih and (not wH):
            self.get_backbone()
            mol = self.mbb
        c1 = mol.GetConformer(-1)
        zs = [ ai.GetAtomicNum() for ai in mol.GetAtoms() ]
        ...
        for match in matches:
            ...
            hj, hk = [ dic_hyb[_a.GetHybridization()] for _a in [aj,ak] ]
            iok1 = ( hj not in [2,3] )
            iok2 = ( hk not in [2,3] )
            ...
            for b1 in aj.GetBonds():
                ...
                for b2 in ak.GetBonds():
                    ...
                    if (l == i):
                        continue
                    dang = rdMolTransforms.GetDihedralDeg(c1, i,j,k,l)
                    ias4 = (i,j,k,l)
                    if key in ['z']:
                        zi,zj,zk,zl = _zs[ias4]
    ...
    def get_angles(self, wH=False, key='z'):
        m2 = self.m
        if self.ih and (not wH):
            self.get_backbone()
            m2 = self.mbb
            ...
            if nneib > 1:
                for i0 in range(nneib):
                    for k0 in range(i0+1,nneib):
                        ...
                        iass3.append(ias)
                        ang = rdMolTransforms.GetAngleDeg(c1, i,j,k)
                        if key in ['ia']:
                            dic[ias] = ang 
                        else:
    ...
    def fixTorsionOpt(self,thresh,maxIters=200,wH=False):
        """
        ...
        if self.forcefield in ['mmff94',]:
            ...
            ff = AllChem.UFFGetMoleculeForceField(mu, mp, \
                        ignoreInterfragInteractions=False)
        for ias_i in self.iass4: #[:1]:
            i,j,k,l = ias_i
            if self.forcefield in ['mmff94',]:
                ff.MMFFAddTorsionConstraint(i,j,k,l,True,-thresh,thresh,9999)
            else:
                ff.UFFAddTorsionConstraint(i,j,k,l,True,-thresh,thresh,9999)
