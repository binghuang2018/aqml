...
class Mol(RawMol):
    ...
    def find_bond_to_N5(self,dvs):
        ...
        if n1>0 or n2>0:
            amap1 = dict(zip(ias1, range(len(ias1))))
            amap2 = dict(zip(ias2, range(len(ias2))))
            amaps = [amap1,amap2];
            self.amap1 = amap1
            self.amap2 = amap2
            sg1 = self.g[ias1][:,ias1]
            # nodes are returned (relative idx)
            iass1 = cg.find_cliques(sg1) if n1>0 else [np.array([],np.int)]
            sg2 = self.g[ias2][:,ias2]
            iass2 = cg.find_cliques(sg2) if n2>0 else [np.array([],np.int)]
            sgs = [sg1,sg2]
            tas = [iass1,iass2]
            self.iass1 = iass1
            self.iass2 = iass2
...
if __name__ == "__main__":
    ...
    import cheminfo.rdkit._rdkit as cir
