...
import cheminfo.rdkit.core as crc
import cheminfo.oechem.amon as coa
import cheminfo.oechem.oechem as coo
import cheminfo.oechem.amon_extend as coae
    ...
    def __init__(self, objs, rcut=4.8, fitmorse=F, property_names=None, \
        ...
        if isinstance(objs,(tuple,list)):
            ...
            for obj in objs:
                ...
                else: # assume cheminfo.core.atoms class
                    #if obj.__class__.__name__ == 'atoms':
                    #    mols.update([obj])
                    #else:
                    raise Exception('Not a class or cheminfo.core.atoms?')
            mols = cc.molecules(fs, property_names)
        else: # assume cheminfo.core.atoms class
            if objs.__class__.__name__ == 'molecules':
                mols = objs
            else:
                raise Exception('Not a class or cheminfo.core.molecules?')
