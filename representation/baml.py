...
class BondEnergies(object):

    def __init__(self, dbf='BE_RDKit.dat'):
        dbf = os.environ['HOME'] + '/Dropbox/workspace/python/cheminfo/' + dbf
        #print ' -- dbf = ', dbf
        assert os.path.exists(dbf), '#ERROR: Bond Energy data file does not exist'
        cs = file(dbf).readlines()
        dic = {}
        for c in cs:
            a1, a2, bo, eb_str = c.split()
            a12 = [a1,a2]; a12.sort()
            key_u = '-'.join( a12 + [ '%.2f'%eval(bo), ] )
            dic[key_u] = eval(eb_str)/c2j # kcal/mol
        bps = list(dic.keys())
        bp = 'N_R-O_3-1.00'
        if bp not in bps:
            dic[bp] = dic[ 'N_R-O_2-1.00' ]
        self.dic = dic
