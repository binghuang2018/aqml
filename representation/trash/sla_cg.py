...
(designed to save time & melting/boiling point prediction)
...
def get_sbop(mbtype, m, zsm=None, local=False, ia=None, normalize=True, sigma=0.05, \
             rcut=4.8, dgrid=0.03, ipot=True, iprt=False, \
...
def get_sbot(mbtype, m, zsm=None, local=False, ia=None, normalize=True, \
             sigma=0.05, label=None, rcut=4.8, dgrid=0.0262, ipot=True, \
             iprt=False, zg=False, pbc='000'):
        ...
        for mbtype in mbtypes:
            ...
            if len(mbtype) == 1:
                ...
                mbsi = get_sbop(mbtype, m, zsm=zsm, normalize=normalize, sigma=sigmas[0], \
                                dgrid=dgrids[0], rcut=rcut, zg=zg, rpower=rpower)[1]
                ...
                mbsi = get_sbot(mbtype, m, zsm=zsm, normalize=normalize, sigma=sigmas[1], \
                                dgrid=dgrids[1], rcut=rcut, zg=zg)[1]
                if alchemy:
                    n3 = len(mbsi)
                    n3_0 = mbs.shape[0]
                    if n3_0 == n1 + n2:
...
def get_slas(ms, h5f=None, mIds=[0,-1], local=False, normalize=True, \
    ...
    for j in range(im1,im2): #enumerate(ms[im1:im2]):
        ...
        Xi,X2N = get_sla(mj, zsm, mbtypes, local = local, normalize=normalize, \
                         ...
                         noprt=noprt, alchemy=alchemy, zg=zg,\
