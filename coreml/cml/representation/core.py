...
from .fcore import fgenerate_coulomb_matrix
from .fcore import fgenerate_unsorted_coulomb_matrix
from .fcore import fgenerate_local_coulomb_matrix
from .fcore import fgenerate_atomic_coulomb_matrix
from .fcore import fgenerate_eigenvalue_coulomb_matrix
from .fcore import fgenerate_bob
...
def py_generate_cm(zs, coords, ds=None, inorm=False, \
                            wz=False, rpower=1.0):
    """ Coulomb matrix
    ...
    if np.any(ds==0):
        ...
        print(' ******** found atom pairs with dij=0: ', list(zip(ias1,ias2)), 'zs1=', zs[ias1], 'zs2=', zs[ias2])
    ...
    ias = np.argsort(L1s)
    return L1s[ias] if inorm else mat[ias,:][:,ias].ravel()
    ...
    if iloc:
        ...
        for ia in ias:
            ...
            for mbtype in mbtypes:
                ...
                if len(mbtype) == 1:
                    ...
                    #print ' 001, pbc = ', pbc
                    mbsi = get_sbop(mbtype, obj, cg=cg, izeff=izeff, iloc=iloc, ia=ia, \
                                    sigma=sigmas[0], dgrid=dgrids[0], rcut=rcut, \
                                    pbc=pbc, rpower=rpower)

                    # multiplication by 1/2 has been done in fslatm.f90
                    ...
                    mbsi = get_sbot(mbtype, obj, cg=cg, izeff=izeff, iloc=iloc, ia=ia, \
        ...
        for mbtype in mbtypes:
            if len(mbtype) == 1:
                ...
                mbsi = get_sbop(mbtype, obj, cg=cg, izeff=izeff, sigma=sigmas[0], \
                ...
                mbsi = get_sbot(mbtype, obj, cg=cg, izeff=izeff, sigma=sigmas[1], \
